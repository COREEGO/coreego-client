...
import { CLOSE_ICON, EYE_ICON, GPS_ICON, MARKER_ICON } from "../../utils/icon";
...
const PlaceMapCard: React.FC<{ place: any, onClose?: () => void }> = ({ place, onClose }) => {
    ...
    return (
        <Card sx={{ width: '100%', height: '100%' }}>
            {
                onClose && <IconButton onClick={onClose} size="small" sx={{ position: 'absolute', right: 1, top: 1 }}>
                    <CLOSE_ICON />
                </IconButton>
            }
            {
                ...
                <Stack flex={1} spacing={2}>
                    ...
                    <Stack direction="row" spacing={2} mt={3} >
                        ...
                        <NavLink target="_blank" to={`https://map.kakao.com/link/to/${place.title},${place.latitude},${place.longitude}`}>
                            <Button startIcon={<GPS_ICON />} size="small" variant="outlined">Naviguer</Button>
                        </NavLink>
                    </Stack>
                </Stack>
            </CardContent>
